...
class ThreeD
{
	constructor(renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera: THREE.PerspectiveCamera, px: number, py: number, ox: number, oy: number, bx: number, by: number) {
	...
	private isInScene(scene: THREE.Scene, object: THREE.Object3D) : boolean {
		let isInScene = false;
	...
	public createShockwave(position: THREE.Vector3)
	{
  ...
  // Set the initial position of the shockwave
	...
	public render(px: number, py: number, ox: number, oy: number, bx: number, by: number) {
	...
	public renderTimer(timer: number){
		const cameraPaths = [
	...
	private _dLight: THREE.DirectionalLight 
	private _aLight: THREE.AmbientLight 
	private _player: THREE.Mesh
	private _opponent: THREE.Mesh
	private _ball: THREE.Mesh;
	private _field: THREE.Mesh;
	private _ball2: THREE.Mesh;
	private _trail: THREE.Points
	private _previousVelocity: THREE.Vector3
	private _currentVelocity: THREE.Vector3
	private _prevBallPos: THREE.Vector3
